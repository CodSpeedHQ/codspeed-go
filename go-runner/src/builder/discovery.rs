...
pub struct GoPackage {
    ...
    /// TBA
    /// "ImportPath": "local.dev/example-complex/internal/config [local.dev/example-complex/internal/config.test]",
...
impl GoBenchmark {
    pub fn new(package_import_path: String, name: String, file_path: PathBuf) -> Self {
        ...
        let import_alias = {
            ...
            format!("{}_{}", name.to_lowercase(), rng.random::<u32>())
        };

        #[cfg(not(test))]
        let import_alias = format!("{}_{}", name.to_lowercase(), rand::random::<u32>());
